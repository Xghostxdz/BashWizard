...
update_packages() {
    if command -v apt &> /dev/null; then
        ...
        log_activity "Packages updated using apt"
    elif command -v pacman &> /dev/null; then
        pacman -Syu --noconfirm
        log_activity "Packages updated using pacman"
    elif command -v dnf &> /dev/null; then
        dnf upgrade --refresh -y
        log_activity "Packages updated using dnf"
    elif command -v yum &> /dev/null; then
        yum update -y
        log_activity "Packages updated using yum"
    else
        echo -e "${R}Unsupported package manager!${RESET}"
    fi
}

install_tools() {
    if command -v apt &> /dev/null; then
        apt install -y curl wget net-tools dnsutils
    elif command -v pacman &> /dev/null; then
        pacman -S --noconfirm curl wget net-tools dnsutils
    elif command -v dnf &> /dev/null; then
        dnf install -y curl wget net-tools bind-utils
    elif command -v yum &> /dev/null; then
        yum install -y curl wget net-tools bind-utils
    fi
...
check_url_safety() {
    echo -e "${Bl}Enter URL to check:${RESET}"
    read -r url
    status_code=$(curl -o /dev/null --silent --head --write-out '%{http_code}' "$url" 2>&1)
    if [ "$status_code" -eq 200 ]; then
        echo -e "${G}The URL is accessible and safe!${RESET}"
        log_activity "Checked URL: $url (Status: $status_code - Safe)"
    else
        echo -e "${R}The URL is not accessible or might be unsafe! (Status: $status_code)${RESET}"
...
# دالة لفحص DNS
check_dns() {
    echo -e "${Bl}Enter URL to check DNS records:${RESET}"
    read -r url
...
check_port() {
    echo -e "${Bl}Enter host to check:${RESET}"
    read -r host
    echo -e "${Bl}Enter port to check:${RESET}"
    read -r port
    nc -zv "$host" "$port" 2>&1
    if [ $? -eq 0 ]; then
        echo -e "${G}Port $port on $host is open.${RESET}"
        log_activity "Checked port $port on $host - Open"
    else
        echo -e "${R}Port $port on $host is closed or inaccessible.${RESET}"
...
chekuser() {
    echo -e "${Bl}Enter your code:${RESET}"
    read -r pss
    filename="$pss"
    ...
    if [ -e "$filename" ]; then
        ...
        echo -e "${RESET}"
        touch "$filename" 
        echo -e "${G}Code '$filename' created successfully.${RESET}"
        clear
        while true; do
...
shell_command() {
    created_file="$1"
    svd=$(basename "$(pwd)")
    o="${G}┌── (${R}${created_file}${G})${G} ~${P}${svd}${Y} ${G}\n└─${R}$ ${W}"
